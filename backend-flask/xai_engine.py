...
def objective_function(weights, cnn_val, swin_val, hybrid_val, target):
    """
    Evaluates how close the weighted sum is to the actual authenticity (target).
    target: 1.0 for authentic, 0.0 for forgery.
    """
    # Normalize weights so they sum to 1.0
    sum_w = np.sum(weights)
    if sum_w == 0: return 1e6 # Avoid division by zero
    ...
    # Return the Error (Squared Error)
...
def run_art_de_optimization(cnn_in, swin_in, hybrid_in, target_val):
    ...
    # 1. Initialization: Random weights between 0 and 1
    ...
    for gen in range(iterations):
        for i in range(pop_size):
            ...
            # Constraint: Keep weights between 0 and 1.0
    ...
    # Return the normalized optimal weights
...
if __name__ == "__main__":
    ...
    try:
        ...
        # In a real test, you know if the painting is authentic (1.0) or fake (0.0)
